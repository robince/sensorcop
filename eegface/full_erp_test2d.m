...
% stim = stim(1:Ntrl);
Nthread = 8;
qrsp = reshape(copnorm_slice_omp_c_double(rspdat(:,:),Nthread),size(rspdat));
qrsp = permute(qrsp, [2 1 3 4]);
%%
% Icop = zeros([Nt Nchan]);
% for ci=1:size(qrsp,2);
%     Icop(ci) = info_gd( qrsp(:,ci), stim, 2, false, true, false );
% end
Icop = reshape(info_cd_slice_nobc_omp(qrsp(:,:,:), 2, int16(stim'+1), 2, Ntrl, Nthread),[Nt Nchan]);
